...
- FINALLY, start producing points for the ROC curve. This is an iterated three-step process.

1. **Pick a threshold.**
2. **Binarize according to the threshold.** Scores above the threshold go to 1; scores below the threshold go to 0.
3. **Compute the false positive rate (FPR) and true positive rate (TPR)** for that threshold.

Let's actually follow this procedure by hand.
...
Cool, right? **The most important metric to extract is in the lower right corner of the plot: the area under the ROC curve (AUROC).** AUROC is the best finger-to-the-wind measure of a classifier's overall quality. The extremal cases are AUROC = 0.5 and AUROC = 1.0. An AUROC of 0.5 indicates a classifier that is no better than random chance. No matter what threshold we choose, the classifier cannot distinguish between the two classes. An AUROC of 1.0 corresponds to a perfect classifier. Even at a false positive rate of zero, the classifier maximizes the true positive rate. This corresponds to the existence of a threshold that perfectly splits all of the classifier's negative and positive predictions.[^1]
...
[^1]: As long as you can split the positive and negative predictions, it kind of doesn't matter whether the positive predictions cluster near 0 and the negative predictions cluster near 1 -- just flip the ground truth labels and you're back to being perfect.